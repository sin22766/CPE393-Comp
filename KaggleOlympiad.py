...
le = LabelEncoder()
transform_y = pd.Series(le.fit_transform(y), name="Status")
...
def objective_xgb(trial):
    ...
    for fold, (train_idx, val_idx) in enumerate(rskf.split(transform_X, transform_y)):
        X_train, X_val = transform_X.iloc[train_idx], transform_X.iloc[val_idx]
        y_train, y_val = transform_y.iloc[train_idx], transform_y.iloc[val_idx]

        # Apply SMOTE with tunable parameters
        try:
            smote = SMOTE(k_neighbors=smote_k, random_state=42)
            X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train)
        except ValueError:
            # If SMOTE fails, fall back to original data
            print(f"SMOTE failed for k={smote_k}, using original data")
    ...
    if len(cv_scores) == 0:
        return float("inf")  # Return a high value if all folds failed
...
# # Step 5: Create and run the Optuna study
# # Start with fewer trials for testing, increase for better results
# number_of_trials = 100

# # Create study and optimize
# study = optuna.create_study(direction="minimize")
# study.optimize(objective_xgb, n_trials=number_of_trials)

# %%
xgb_params = {
    "objective": "multi:softprob",
    "eval_metric": "mlogloss",
    "lambda": 1.1369029459700144e-06,
    "alpha": 0.012063715109367643,
    "max_depth": 6,
    "eta": 0.0016842485569386354,
...
# %%
for fold, (train_idx, test_idx) in enumerate(rskf.split(transform_X, transform_y)):
    print(f"Starting {fold}")
    X_train, X_val = transform_X.iloc[train_idx], transform_X.iloc[test_idx]
    y_train, y_val = transform_y.iloc[train_idx], transform_y.iloc[test_idx]
    ...
    val_loss.append(log_loss(y_val, val_pred_prob))
    train_loss.append(log_loss(y_train, train_pred_prob))

# %%
sns.lineplot(data=log_score)

# %%
submit_pred = clf.predict_proba(X_test)
submit_pred = pd.DataFrame(submit_pred, columns=["Status_C", "Status_CL", "Status_D"])
submit_pred = pd.concat([raw_test["id"], submit_pred], axis=1)
submit_pred
